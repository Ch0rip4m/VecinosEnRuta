...
class UsuarioManager(BaseUserManager):
    ...
    def create_superuser(
        self, email, nombre_usuario, apellido_usuario, password, **extra_fields
    ):
        ...
        return self.create_user(
            email, nombre_usuario, apellido_usuario, password, **extra_fields
        )
...
class Usuario(AbstractBaseUser, PermissionsMixin):
    ...
    apellido_usuario = models.CharField(
        max_length=255, verbose_name="Apellido del usuario"
    )
    ...
    email = models.EmailField(
        max_length=255, verbose_name="Correo electronico", default="", unique=True
    )
    ...
    descripcion_usuario = models.TextField(
        max_length=1000, verbose_name="Descripcion del usuario"
    )
    imagen_perfil = models.ImageField(
        upload_to="perfil/",
        null=True,
        blank=True,
        verbose_name="Imagen de perfil Usuarios",
    )
    tiempo_registro = models.DateTimeField(
        auto_now_add=True, verbose_name="Registro de creacion"
    )
    ...
    def assign_roles(self, roles):
        ...
        roles_actuales = RolUsuario.objects.filter(id_usuario=self).values_list(
            "id_rol__nombre_rol", flat=True
        )

        # Filtrar los roles que deben ser agregados
        nuevos_roles = [
            role_name for role_name in roles if role_name not in roles_actuales
        ]

        # Filtrar los roles que deben ser eliminados
        roles_a_eliminar = [
            role_name for role_name in roles_actuales if role_name not in roles
        ]
    ...
    def assign_location(self, location):
        # Obtener los roles actuales del usuario
        comuna_actual = ComunaUsuario.objects.filter(id_usuario=self).values_list(
            "id_comuna__nombre_comuna", flat=True
        )

        # Filtrar los roles que deben ser agregados
        nueva_comuna = [
            nombre_comuna
            for nombre_comuna in location
            if nombre_comuna not in comuna_actual
        ]

        # Filtrar los roles que deben ser eliminados
        comuna_a_eliminar = [
            nombre_comuna
            for nombre_comuna in comuna_actual
            if nombre_comuna not in location
        ]
...
class Comunidades(models.Model):
    ...
    nombre_comunidad = models.CharField(
...
class ComunaComunidad(models.Model):
    ...
    id_comunidad = models.ForeignKey(
        Comunidades,
        to_field="id_comunidad",
        on_delete=models.CASCADE,
        verbose_name="ID de la comunidad",
    )
...
class MiembrosComunidad(models.Model):
    ...
    def __str__(self):
        return f"{self.id_comunidad} - {self.id_miembro}"
...
class RolUsuario(models.Model):
    ...
    id_rol = models.ForeignKey(
        Roles, to_field="id_rol", on_delete=models.CASCADE, verbose_name="ID del rol"
    )
...
class Vehiculos(models.Model):
    ...
    modelo_vehiculo = models.CharField(
        max_length=50, verbose_name="Modelo del vehiculo"
    )
    tipo_de_vehiculo = models.CharField(max_length=50, verbose_name="Tipo de vehiculo")
    nro_asientos_disp = models.IntegerField(
        verbose_name="Numero de asientos disponibles"
    )
    ...
    imagen_perfil = models.ImageField(
        ...
        null=True,
        blank=True,
        verbose_name="Imagen de perfil vehiculo",
    )
    tiempo_registro = models.DateTimeField(
        auto_now_add=True, verbose_name="Registro de creacion"
    )
...
class VehiculoUsuario(models.Model):
    ...
    id_vehiculo = models.ForeignKey(
        Vehiculos,
        to_field="id_vehiculo",
        on_delete=models.CASCADE,
        verbose_name="ID del vehiculo",
    )
...
class Calificaciones(models.Model):
    id_calificacion = models.AutoField(
        primary_key=True, verbose_name="ID de la calificacion"
    )
    id_calificador = models.ForeignKey(
        Usuario,
        to_field="id_usuario",
        related_name="calficacion_realizada",
        on_delete=models.CASCADE,
        verbose_name="ID del usuario calificador",
    )
    id_calificado = models.ForeignKey(
        Usuario,
        to_field="id_usuario",
        related_name="calficacion_recibida",
        on_delete=models.CASCADE,
        verbose_name="ID del usuario calificado",
    )
    comentario = models.TextField(
        max_length=500, verbose_name="Comentarios de la calificacion"
    )
    fecha_calificacion = models.DateTimeField(
        auto_now_add=True, verbose_name="Fecha de la calificacion"
    )
...
class CategoriasCalificacion(models.Model):
    id_calificacion = models.ForeignKey(
        Calificaciones,
        to_field="id_calificacion",
        on_delete=models.CASCADE,
        verbose_name="ID de la calificacion",
    )
...
class Rutas(models.Model):
    ...
    hora_salida = models.CharField(
        max_length=10, verbose_name="Hora de salida de la ruta"
    )
    cupos = models.IntegerField(verbose_name="Cupos disponibles")
    tiempo_registro = models.DateTimeField(
        auto_now_add=True, verbose_name="Fecha de creacion de ruta"
    )
    ...
    def assign_days(self, days):
        # Obtener los roles actuales del usuario
        dias_actuales = DiasRutas.objects.filter(id_ruta=self).values_list(
        ...
        # Filtrar los roles que deben ser eliminados
        dias_a_eliminar = [
            day_name for day_name in dias_actuales if day_name not in days
        ]
...
class MiembrosRuta(models.Model):
    id_ruta = models.ForeignKey(
        Rutas, to_field="id_ruta", on_delete=models.CASCADE, verbose_name="ID ruta"
    )
    id_miembro = models.ForeignKey(
        Usuario,
        to_field="id_usuario",
...
class DiasRutas(models.Model):
    id_dia = models.ForeignKey(
        ...
        to_field="id_dia",
        on_delete=models.CASCADE,
        verbose_name="ID del dia de la semana",
    )
    id_ruta = models.ForeignKey(
        Rutas,
        to_field="id_ruta",
...
class OrdenTrayectoria(models.Model):
    id_trayectoria = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        verbose_name="ID de la trayectoria",
    )
...
class Notificaciones(models.Model):
    id_notificacion = models.AutoField(
        primary_key=True, verbose_name="ID de la notificacion"
    )
    id_propietario = models.ForeignKey(
        Usuario,
        to_field="id_usuario",
        related_name="propietario_ruta",
        on_delete=models.CASCADE,
        verbose_name="ID propietario ruta o comunidad",
    )
    id_solicitante = models.ForeignKey(
        Usuario,
        to_field="id_usuario",
        related_name="propietario_solicitud",
        on_delete=models.CASCADE,
        verbose_name="ID del solicitante",
    )
    id_comunidad = models.ForeignKey(
        Comunidades,
        to_field="id_comunidad",
        on_delete=models.CASCADE,
        verbose_name="ID de comunidad",
        null=True,
        blank=True,
    )
    id_ruta = models.ForeignKey(
...
class ContactosEmergencia(models.Model):
    ...
    correo_emergencia = models.CharField(
        max_length=50, verbose_name="Correo de emergencia"
    )
