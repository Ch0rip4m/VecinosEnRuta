    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if refresh and access:
                response.set_cookie(
                    "access", access, httponly=True, path="/", samesite="Lax"
                )
                response.set_cookie(
                    "refresh", refresh, httponly=True, path="/", samesite="Lax"
                )
                return response
            else:
                return Response(
                    {"detail": "Tokens could not be generated"},
...
class VerifyTokenExistView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if access_token and refresh_token:
            ...
            return Response(
                {"detail": "Tokens do not exist"}, status=status.HTTP_401_UNAUTHORIZED
            )
